...
function AlternateDataCenterModal({ isOpen, toggle, onSelect }) {
  ...
  const fetchLocations = async () => {
    try {
      setLoading(true);
      console.log("🔍 Fetching Alternate Data Centers from API...");
  ...
  return (
    <Modal isOpen={isOpen} toggle={toggle} size="lg">
      <ModalHeader toggle={toggle}>Select Alternate/Failover Data Center</ModalHeader>
      <ModalBody>
        {loading ? (
          <p>Loading...</p>
        ) : locations.length > 0 ? (
...
export default AlternateDataCenterModal;
